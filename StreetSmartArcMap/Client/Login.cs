...
namespace StreetSmartArcMap.Client
{
    ...
    public class Login
    {
        ...
        {
            get { return Path.Combine(FileUtils.FileDir, "GSClientLogin.xml"); }
        }
        ...
        public bool Check()
        {
            HasCredentials = !string.IsNullOrEmpty(Username) && !string.IsNullOrEmpty(Password) && GlobeSpotterConfiguration.CheckCredentials();
        ...
        private static Aes CreateAes(string password, byte[] salt)
        {
            ...
            {
                //throw new ArgumentException(System.Resources.Login_CreateAes_Password_must_be_at_least_8_characters_, "password");
                throw new ArgumentException("Password must be at least 8 characters long", "password");
            }

            if (salt.Length < 8)
            {
                //throw new ArgumentException(System.Resources.Login_CreateAes_Salt_must_be_at_least_8_bytes_, "salt");
                throw new ArgumentException("AES Salt must be at least 8 bytes", "salt");
            }
