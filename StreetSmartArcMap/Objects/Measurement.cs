...
namespace StreetSmartArcMap.Objects
{
    class Measurement : SortedDictionary<int, MeasurementPoint>, IDisposable
    {
        ...
        public void UpdatePoint(int pointId, IFeature measurementData, int index)
        {
        ...
        {
            return _StreetSmartDockableWindow?.GetMeasurementPointIndex(_entityId, pointId) ?? 0;
        }
        ...
        {
            if (entityId > -1 && Measurements.ContainsKey(entityId))
                return Measurements[entityId];
            else
                return null;
        }
