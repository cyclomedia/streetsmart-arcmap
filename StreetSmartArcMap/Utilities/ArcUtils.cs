...
namespace StreetSmartArcMap.Utilities
{
    internal class ArcUtils
    {
        ...
        public static ISymbol GetPipSymbol(int sizeLayer, Color color)
        {
            ...
            string writePath = System.IO.Path.Combine(tempPath, string.Format("{0}pip.bmp", color.Name));
            bitmap8B.Save(writePath, ImageFormat.Bmp);
            IPictureMarkerSymbol symbol = (IPictureMarkerSymbol)new PictureMarkerSymbol();
        ...
        public static ISymbol GetForbiddenSymbol(int sizeLayer, Color color)
        {
            ...
            Bitmap bitmap8B = bitmap.To8BppIndexed();
            string tempPath = System.IO.Path.GetTempPath();
            string writePath = System.IO.Path.Combine(tempPath, string.Format("{0}forbidden.bmp", color.Name));
            bitmap8B.Save(writePath, ImageFormat.Bmp);
            IPictureMarkerSymbol symbol = (IPictureMarkerSymbol)new PictureMarkerSymbol();
            symbol.CreateMarkerSymbolFromFile(esriIPictureType.esriIPictureBitmap, writePath);
            symbol.Size = size075;
